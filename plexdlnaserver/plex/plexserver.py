...
from ..utils import g, plex_server_response_headers, timeline_poll_headers, xml2dict
from . import pin_login
from .adapters import adapter_by_device
from .gdm import PlexGDM
from .subscribe import sub_man
...
async def on_new_dlna_device(location_url):
    if not location_url.startswith("http://192.168.1.25:"):
        return

    logger.info("got new DLNA device %s", location_url)
    for device in devices:
        if device.location_url == location_url:
            return
    ...
    try:
        ...
        logger.warning(
            "failed to get DLNA device data %s. Reason: %s", location_url, exc
        )
        return

    logger.info("got new DLNA device %s", device.name)
...
def guess_host_ip(request: Request):
    ...
    if not request.url.hostname:
        raise Exception("request url has no hostname")

    if request.url.hostname and request.url.hostname.startswith("127.0.0"):
        logger.warning("request url hostname is localhost")
        return
...
async def link_page(request: Request):
    ...
    for device in devices:
        ...
        if adapter.plex_bind_token is not None:
            found_devices.append(dict(name=device.name, uuid=device.uuid, binded=True))
        else:
            pin, pin_id = await pin_login.get_pin(device)
    ...
    return templates.TemplateResponse(
        "bind.html", {"devices": found_devices, "request": request}
    )
...
async def dlna_subscribe(request: Request, uuid: str):
    logger.info("NOTIFY /dlna/callback/%s", uuid)
    adapter = adapter_by_device(await get_device_by_uuid(uuid))
    b = await request.body()
    info = xml2dict(b)

    if info.get("RelativeTimePosition"):
        logger.info(
            "got playback info %s %s %s",
...
async def timeline_poll(
    ...
    if waiting_poll_count > 3:
        logger.info("waiting poll %s", waiting_poll_count)
    ...
    adapter = adapter_by_device(device)
    if wait == 1:
        await adapter.wait_for_event(
            settings.plex_notify_interval * 20,
            interesting_fields=["state", "volume", "current_uri", "elapsed_jump"],
        )
    msg = await sub_man.msg_for_device(device)

    while msg is None:
        logger.info("waiting for msg %s", target_uuid)
    ...
    if datetime.utcnow() - begin_time >= timedelta(milliseconds=500):
        logger.info("long poll %s", target_uuid)
        logger.info("%s used %s", request.url, datetime.utcnow() - begin_time)
    waiting_poll_count -= 1

    asyncio.create_task(sub_man.notify_server_device(device, force=True))

    return await build_response(
        msg, device=device, headers=timeline_poll_headers(device)
    )
...
async def resources(
    ...
    logger.info("resources for %s", device.name)
    res = "<MediaContainer>"
    res += (
        f'<Player title="{device.name}" protocol="plex" protocolVersion="1" '
...
def start_plex_server(port: int | None = None):
    logging.basicConfig(level=logging.DEBUG)
    logger.info("starting plex server")

    return uvicorn.run(
        "plexdlnaserver.plex.plexserver:plex_server",
        host="0.0.0.0",

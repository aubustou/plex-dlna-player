...
from .play_queue import TrackInfo
...
class PlexLibInfo(TypedDict):
            protocol: str
            address: str
            port: int | str
            machineIdentifier: str
class StateInfo(TrackInfo, PlexLibInfo, total=False):
    state: Literal["playing", "paused", "stopped"] | None
    time: int | str
    volume: int
    mute: bool
    shuffle: int
...
def adapter_by_device(device, query_params: QueryParams | None = None):
...
class PlexLib:
    ...
    def __post_init__(self):
        self.fix_plex_direct()

    def fix_plex_direct(self):
        if self.address.endswith(".plex.direct"):
            ip = IPv4Address(self.address.split(".")[0].replace("-", "."))
            if ip.is_private:
                self.address = str(ip)
                self.protocol = "http"
...
class DlnaState(object):
    ...
    def start_looping(self):
        ...
        self.looping_thread = Thread(
            target=self.background_loop, name=f"Dlna State Thread {str(self.dlna)}"
        )
    ...
    def __repr__(self):
        return (
            f"{self.dlna.name}: state {self.state} {self.elapsed} {self.volume} "
    ...
    async def check(self, client: aiohttp.ClientSession, check_count=0):
        ...
        if (
            check_count % state_check_count == 0
        ...
        if position_info and position_info.result:
            ...
            self.elapsed = int(
                parse_timedelta(position_info.RelTime).total_seconds() * 1000
            )
            ...
            if (
                not state
                and not self._changed_state
                and self.state in {"TRANSITIONING", "PLAYING"}
            ):
                ...
                try:
                    ...
                    logger.warning(
                        "DLNA %s state retry failed. Passing", self.dlna.name
                    )
        ...
        if volume and volume.result:
            ...
            self.volume = convert_volume(
                volume, self.dlna.volume_max, self.dlna.volume_min, 100, 0, 1
            )
        ...
        if changed_state and self.state_change_callback:
            logger.debug("%s check loop %s %s", self.dlna.name, changed_state, self)
            self.state_change_callback(changed_state)

    @property
    def loop_interval(self):
        if datetime.utcnow() - self.last_access_time >= timedelta(
            seconds=90
    ...
    async def wait_for_next_loop(self):
        try:
            await asyncio.wait_for(
                self.looping_wait_event.wait(), timeout=self.loop_interval
            )
        except Exception:
            if False:
                logger.debug("DLNA %s state loop wait error. Passing", self.dlna.name)
        self.looping_wait_event.clear()

    async def _check_loop(self):
        logger.info("DLNA %s state loop started", self.dlna.name)
        ...
        logger.info("DLNA %s state loop stopped", self.dlna.name)
    ...
    def update(self, state: str = "", uri: str = "", position: str = ""):
        ...
        if (
            (state == "" or self.state == state)
            and (uri == "" or self.current_uri == uri)
            and (elapsed == "" or self.elapsed == elapsed)
        ):
            return
        if self.running_loop is None:
            logger.info(
                "DLNA %s state update discard due to no running loop %s %s %s",
                self.dlna.name,
                state,
                uri,
                position,
            ...
            return
        if current_thread() == self.looping_thread:
            self.running_loop.create_task(
                self.update_in_thread(state=state, uri=uri, elapsed=elapsed)
            )
        else:
    ...
    async def update_in_thread(self, state="", uri="", elapsed=""):
        ...
        if self.state == state and self.current_uri == uri and self.elapsed == elapsed:
            return
        logger.debug(
            "DLNA %s real update state from sub %s %s %s",
            self.dlna.name,
            state,
            uri,
            elapsed,
        ...
        async with self.change_session_lock:
            logger.debug(
                "DLNA %s real update state from sub in lock %s %s %s",
                self.dlna.name,
                state,
                uri,
                elapsed,
...
class PlexDlnaAdapter:
    def __init__(self, dlna, query: QueryParams | None = None):
        logger.info("init adapter for %s", dlna.name)
    ...
    def check_auto_next(self, changed: DotMap):
        ...
        if (
            changed.state
            and changed.state != "PLAYING"
            and changed.old.state == "TRANSITIONING"
        ):
            return False
        ...
        async def auto_next():
            if self.queue.repeat == 1:
                ...
                self.queue.repeat == 2
                and (await self.queue.selected_offset())
                >= (await self.queue.total_count() - 1)
                and self.shuffle == 0
            ):
        ...
        if (
            self.state.current_uri is not None
            and not changed.state
            and not changed.uri
            ...
            if (
                ...
                == 0
                < changed.old.elapsed
                <= self.current_track_info.duration
                and self.current_track_info.duration - changed.old.elapsed <= 2000
            ) or (
                changed.elapsed
                and changed.elapsed > changed.old.elapsed
                and self.current_track_info.duration // 1000 * 1000
                <= changed.elapsed
                <= self.current_track_info.duration
            ):
                self.no_notice = True
                logger.info("auto next stopped %s", self.state.state)
                logger.info("Elapsed %s", changed.elapsed)
                logger.info("Duration %s", self.current_track_info.duration)
            ...
            not changed.uri
            and changed.old.state == "PLAYING"
            and changed.state == "STOPPED"
            and self.state.current_track_duration - self.state.elapsed <= 1
        ):
            self.no_notice = True
            logger.info(
                "auto next transitioning %s %s", changed.old.state, changed.state
            )
            self.state.update(state="TRANSITIONING", uri=None)
            asyncio.run_coroutine_threadsafe(auto_next(), self.loop)
            self.no_notice = False
    ...
    def state_changed_callback(self, changed_state: DotMap):
        if self.loop.is_closed():
            return
        if (
            "elapsed" not in changed_state.keys()
            or len(changed_state.keys()) > 2
            or not (0 <= changed_state.elapsed - changed_state.old.elapsed <= 1000)
        ):
            logger.info(
                "DLNA %s state change %s", self.dlna.name, changed_state.toDict()
            )
        n = self.check_auto_next(changed_state)
        if not n:
            asyncio.run_coroutine_threadsafe(
                self.state_changed(changed_state), self.loop
            )
    ...
    async def state_changed(self, changed_state: DotMap):
        ...
        for e in self.wait_state_change_events:
            ...
            if "elapsed_jump" in e["interesting_fields"]:
                if "elapsed" in changed_state and not (
                    0 <= changed_state.elapsed - changed_state.old.elapsed <= 1000
                ):
                    e["event"].set()
                    removed_event.append(e)
                    continue
    ...
    async def wait_for_event(self, timeout=None, interesting_fields=None):
        event = asyncio.Event()
        self.wait_state_change_events.append(
            dict(event=event, interesting_fields=interesting_fields)
        )
    ...
    async def next(self, revert=False):
        ...
        if current_offset >= await self.queue.total_count() or current_offset < 0:
            ...
            return
        self.state.update(state="TRANSITIONING")
        logger.info("next %s", current_offset)
    ...
    async def set_volume(self, volume):
        volume = convert_volume(
            ...
            100,
            0,
            self.dlna.volume_max,
            self.dlna.volume_min,
            self.dlna.volume_step,
        )
    ...
    async def update_plex_tv_connection(self):
        ...
        if not self.plex_bind_token:
            ...
            if not self.plex_bind_token:
                return
        await g.http.put(
            f"https://plex.tv/devices/{self.dlna.uuid}?X-Plex-Token={self.plex_bind_token}",
            data={
                "Connection[][uri]": f"http://{settings.host_ip}:{settings.http_port}"
            },
    ...
    def update_state(self, info):
        ...
        if not state and not uri and not pos:
            logger.info("ignoring notice no info")
            return
        ...
        logger.debug("update state from sub %s %s %s", state, uri, pos)
